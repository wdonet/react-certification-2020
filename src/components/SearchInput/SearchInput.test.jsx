...
describe('SearchInput', () => {
  it('renders with initial word to search', () => {
    render(<SearchInput initSearchWord="wizeline" />);
    expect(screen.getByDisplayValue('wizeline')).toBeTruthy();
  });

  it('triggers change on enter', () => {
    const onChange = jest.fn();
    const searchWord = 'wizeline';

    render(<SearchInput initSearchWord={searchWord} onChange={onChange} />);

    fireEvent.keyDown(screen.getByRole('textbox'), { key: 'Enter' });

    expect(onChange).toHaveBeenCalledWith(searchWord);
  });
